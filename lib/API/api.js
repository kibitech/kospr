...
app.post('/api/token', function(req, res) {  	
 	//Validate user
	connection.query('SELECT user_id AS id, user_name AS username, user_pass AS password FROM k_users WHERE user_name = "'+req.headers.username+'"', function (error, results, fields) {
	  if (error) throw error;	  
		if(results.length != 0)
		{			
			if (req.headers.username === results[0].username && md5(req.headers.password) === results[0].password ) {
...
app.get('/login/:data', ensureToken, function(req, res){
	jwt.verify(req.token, jwt_secret, function(err, data) {
	    if (err) {
	      res.sendStatus(403);
	    } else {
			  ...
			  {			  	
			  	...
			  	{
			  		return res.json({login:true,user:{id:results[0].id,username:results[0].username,email:results[0].email,type:results[0].type,date:results[0].date,status:results[0].status}});
			  	}			  				  	
			  	else
			  	{
