...
//http://localhost:4000/user/del/1
app.get('/user/del/:id', ensureToken, function(req, res){
	jwt.verify(req.token, jwt_secret, function(err, data) {
	    if (err) {
	      res.sendStatus(403);
	    } else {
	      	var id = req.param('id');
		  	connection.query('UPDATE k_users SET user_status = 0 WHERE user_id = '+id, function (error, results, fields) {
			  if (error) throw error;	  
			  if(error)
			  {			  	
			  	return res.json({user_del:false});
			  }
			  else
			  {
			  	return res.json({user_del:true});	
			  }
			});	
	    }
	});	
	    ...
	    if (err) {
	      res.sendStatus(403);
			...
			var date = new Date();
			  ...
			  {
			  	var last_segment = results.insertId;
			  	return res.json({segment_add:true,last_segment:last_segment});	
			  }
			});
	    }
	});		
...
app.get('/node/:id', ensureToken , function(req, res){		
  	jwt.verify(req.token, jwt_secret, function(err, data) {
	    if (err) {
	      res.sendStatus(403);
	    } else {
	    	var id = req.param('id');
	      connection.query('SELECT n.node_id AS id, n.node_nodeKey AS nodekey, n.node_sku AS sku, n.node_manufacturer AS manufacturer, n.node_version AS version, n.node_segment AS segment_id, s.segment_name AS segment_name ,n.node_registered_by AS user_id, u.user_name AS username, n.node_date AS date, n.node_status AS status FROM  (k_nodes n LEFT JOIN k_segments s ON n.node_segment = s.segment_id) LEFT JOIN k_users u ON u.user_id = n.node_registered_by WHERE n.node_status = 1 AND n.node_id = '+id, function (error, results, fields) {
			  if (error) throw error;
			  if(results.length != 0)
			  {	  				  	
			  	return res.json(results);
	...
	jwt.verify(req.token, jwt_secret, function(err, data) {
	    if (err) {
	      ...
	      var data = req.param('data');	
				var json = JSON.parse(data);
				var date = new Date();
				var nodekey = generator(13);
		  	var date_format = date.getFullYear()+'-'+date.getMonth()+'-'+date.getDate()+' '+date.getHours()+':'+date.getMinutes()+':'+date.getSeconds();	    
		  	connection.query('INSERT INTO k_nodes(node_nodeKey,node_sku,node_manufacturer,node_version,node_registered_by,node_segment,node_date,node_status) VALUES("'+nodekey+'","'+json.sku+'","'+json.manufacturer+'","'+json.version+'",'+json.user+', '+json.segment+',"'+date_format+'",1)', function (error, results, fields) {
			  if (error) throw error;
			  if(error)
			  {	  	
			  ...
			  {
			  	var last_node = results.insertId;
			  	return res.json({node_add:true,last_node:last_node,nodekey:nodekey});	
			  }
			});	
	    }
	});	
